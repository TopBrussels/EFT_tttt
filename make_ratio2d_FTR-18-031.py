...
if do1d :
    ...
    for histowork in histo1d13,histo1d14,histo1d27:
        ...
        for workval in thresholdval0[0],thresholdval1[0],thresholdval1[1],thresholdval2[0],thresholdval2[1],thresholdval3[0]:
...
for combo in itertools.combinations(range(0,5),2) :
    ...
    c1=myfunc.GetParameter(2)-thresholdval0[0]-1
    det1=rt.TMath.Sqrt((b1*b1)-(4*a1*c1))

    histo1d2.Fit(myfunc,"Q","goff")
    a2=myfunc.GetParameter(0)
    b2=myfunc.GetParameter(1)
    c2=myfunc.GetParameter(2)-thresholdval0[0]-1
    det2 =rt.TMath.Sqrt((b2*b2)-(4*a2*c2))
    ...
    max_range=[1.2*workrange[0],2.5*workrange[1]]
...
#    print max_range,min_range,step_size
    ...
    hist2d = rt.TH2D('hist2d_'+str(combo[0])+str(combo[1]),'fractional difference 13 TeV;'+wilsoncoeff_names[combo[0]]+' [GeV^{-2}];'+wilsoncoeff_names[combo[1]]+' [GeV^{-2}]',number_of_points_in_direction[0],min_range[0]-step_size[0]/2.,max_range[0]+step_size[0]/2.,
    ...
    scenario0a=hist2d.Clone("scenario0a_"+str(combo[0])+str(combo[1]))
    
    scenario1=hist2d.Clone("scenario1_"+str(combo[0])+str(combo[1]))
    scenario1a=hist2d.Clone("scenario1a_"+str(combo[0])+str(combo[1]))

    scenario2=hist2d.Clone("scenario2_"+str(combo[0])+str(combo[1]))
    scenario2a=hist2d.Clone("scenario2a_"+str(combo[0])+str(combo[1]))
    
    scenario3=hist2d.Clone("scenario3_"+str(combo[0])+str(combo[1]))
    
    print scenario0,scenario1,scenario2,scenario3
    ...
    for i in range(0,number_of_points_in_direction[0]+1):
        for j in range(0,number_of_points_in_direction[0]+1):
            ...
            if eft13.gen_eft_xs(workarray)/eft13.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval0[0] :
                #            print i,j,x,y,xs
                scenario0.SetBinContent(i,j,0.)
            else:
                scenario0.SetBinContent(i,j,1.)
            if eft14.gen_eft_xs(workarray)/eft14.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval1[0] :
                #            print i,j,x,y,xs
                scenario1.SetBinContent(i,j,0.)
            else:
                scenario1.SetBinContent(i,j,1.)
            if eft14.gen_eft_xs(workarray)/eft14.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval1[1] :
                #            print i,j,x,y,xs
                scenario1a.SetBinContent(i,j,0.)
            else:
                scenario1a.SetBinContent(i,j,1.)
            if eft14.gen_eft_xs(workarray)/eft14.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval2[0] :
                #            print i,j,x,y,xs
                scenario2.SetBinContent(i,j,0.)
            else:
                scenario2.SetBinContent(i,j,1.)
            if eft14.gen_eft_xs(workarray)/eft14.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval2[1] :
                #            print i,j,x,y,xs
                scenario2a.SetBinContent(i,j,0.)
            else:
                scenario2a.SetBinContent(i,j,1.)
            if eft27.gen_eft_xs(workarray)/eft27.gen_eft_xs([0,0,0.,0.,0.]) < 1.+thresholdval3[0] :
                #            print i,j,x,y,xs
                scenario3.SetBinContent(i,j,0.)
            else:
                scenario3.SetBinContent(i,j,1.)

    legendstring0 = "LHC 13TeV, #Delta#sigma_{tttt} : "+str(round(100*thresholdval0[0],0))+"% (reference)"
    legendstring1 = "HL-LHC 0.3 ab^{-1}, #Delta#sigma_{tttt} : "+str(round(100*thresholdval1[0],0))+"% (stat)"
    legendstring1a = "HL-LHC 0.3 ab^{-1}, #Delta#sigma_{tttt} : "+str(round(100*thresholdval1[1],0))+"% (stat+syst)"
    legendstring2 = "HL-LHC 3 ab^{-1}, #Delta#sigma_{tttt} : "+str(round(100*thresholdval2[0],0))+"% (stat)"
    legendstring2a = "HL-LHC 3 ab^{-1}, #Delta#sigma_{tttt} : "+str(round(100*thresholdval2[1],0))+"% (stat+syst)"
    legendstring3 = "HE-LHC 15 ab^{-1}, #Delta#sigma_{tttt} : "+str(round(100*thresholdval3[0],0))+"% (stat)"

    hist2d.SetTitle("")
    #hist2d.Draw("CONT1Z")
    #rt.gPad.SetLogz()
    hist2d.Draw("axis")
    scenario0.SetLineColor(rt.kGray)
    scenario0.Draw("CONT3SAME")
    scenario1.SetLineColor(rt.kAzure+1)
    scenario1.Draw("CONT3SAME")
    scenario1a.SetLineColor(rt.kAzure+0)
    scenario1a.Draw("CONT3SAME")
    scenario2.SetLineColor(rt.kGreen+3)
    scenario2.Draw("CONT3SAME")
    scenario2a.SetLineColor(rt.kGreen+2)
    scenario2a.Draw("CONT3SAME")
    scenario3.SetLineColor(rt.kRed)
    scenario3.Draw("CONT3SAME")
    rt.gPad.Update()
    leg = rt.TLegend(0.49,0.61,0.96,0.93)
    leg.SetFillStyle(0)
    leg.SetBorderSize(0)
    leg.AddEntry(scenario0,legendstring0,"l")
    leg.AddEntry(scenario1,legendstring1,"l")
    leg.AddEntry(scenario1a,legendstring1a,"l")
    leg.AddEntry(scenario2,legendstring2,"l")
    leg.AddEntry(scenario2a,legendstring2a,"l")
    leg.AddEntry(scenario3,legendstring3,"l")
    leg.SetEntrySeparation(0.5*leg.GetEntrySeparation())
    leg.SetMargin(0.5*leg.GetMargin())
    leg.Draw("same")
    ...
    rt.gPad.Print("plot"+str(combo[0])+str(combo[1])+".root")
    outfile.cd()
hist2d.Write()
outfile.Close()
